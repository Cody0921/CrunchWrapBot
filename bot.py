...
async def menu(interaction: discord.Interaction, category: str = None):
    ...
    if not data:
        ...
        return
    # send a simple embed list
    embed = discord.Embed(title=f"Menu{(' - ' + category) if category else ''}")
    for it in data[:8]:
        name = it.get('name')
        desc = it.get('description') or ''
        price = it.get('price')
        embed.add_field(name=f"{name} — ${price:.2f}", value=desc or "No description", inline=False)
...
async def item(interaction: discord.Interaction, item_name: str):
    await interaction.response.defer()
    url = f"{API_BASE}/menu/item/{item_name}"
    async with aiohttp.ClientSession() as session:
        try:
    ...
    embed.add_field(name="Price", value=f"${data.get('price'):.2f}", inline=True)
...
async def order_view(interaction: discord.Interaction):
    ...
    for it in items:
        embed.add_field(name=f"{it['name']} x{it['quantity']}", value=f"Subtotal: ${it['subtotal']:.2f}", inline=False)
